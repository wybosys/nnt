...
static void check_cell_alloced(pointer p, int expect_alloced)
{
  ...
  if(typeflag(p) & !expect_alloced)
    {
      fprintf(MSGIO,"Cell is already allocated!\n");
    }
  if(!(typeflag(p)) & expect_alloced)
    {
      fprintf(MSGIO,"Cell is not allocated!\n");
    }
...
static INLINE int skipspace(scheme *sc) {
    int c = 0, curr_line = 0; curr_line = curr_line;
...
static pointer _Error_1(scheme *sc, const char *s, pointer a) {
    ...
    FILE *file = sc->load_stack[sc->file_i].rep.stdio.file;
    if(file && file != stdin) {
...
int main(int argc, char **argv) {
  ...
  if(!scheme_init(&sc)) {
    fprintf(MSGIO,"Could not initialize!\n");
    return 2;
  }
  ...
  do {
    ...
    if(isfile && fin==0) {
      fprintf(MSGIO,"Could not open file %s\n",file_name);
    } else {
      if(isfile) {
      ...
      if(!isfile || fin!=stdin) {
    if(sc.retcode!=0) {
      fprintf(MSGIO,"Errors encountered reading %s\n",file_name);
    }
    if(isfile) {
      fclose(fin);
    }
