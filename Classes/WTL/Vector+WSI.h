...
template <typename valT>
inline_impl usize length(wtl::vector<valT> const& con)
{
    return con.size();
}

template <typename valT>
inline_impl wtl::vector<valT>& append(wtl::vector<valT>& con, valT const* buf, usize len)
{
    usize old = con.size();
    con.resize(old + len);
...
static wtl::vector<valT> tovector(conT& sck)
{  
}

template <typename valT, typename valT2>
static wtl::vector<valT> tovector(wtl::stack<valT2> sck)
{
    wtl::vector<valT> re;
    while (sck.size())
    {
...
template <typename valT, typename valT2>
static wtl::vector<valT> tovector(wtl::stack<valT2> const* psck)
{
    wtl::vector<valT> re;
    if (psck == NULL)
        return re;
    
    wtl::stack<valT2> sck = *psck;
    while (sck.size())
    {
        re.push_back((valT)sck.top());
        sck.pop();
...
template <class valT>
inline_impl ::wsi::wtl::vector<valT const*>& operator << (::wsi::wtl::vector<valT const*>& con, valT* p)
{
