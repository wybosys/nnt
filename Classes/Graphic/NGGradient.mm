...
@interface NgGradient()

@property (nonatomic, retain, readwrite) NgColorSpace *colorspace;
@property (nonatomic, assign, readwrite) NgGradientBlendingMode blendingMode;

-(void)commonInit;
-(void)addElement:(NgGradientElement*)newElement;
...
-(NgGradientElement *)elementAtIndex:(NSUInteger)index;

-(NgGradientElement)removeElementAtIndex:(NSUInteger)index;
-(NgGradientElement)removeElementAtPosition:(CGFloat)position;
...
- (id)init {
    ...
    self.blendingMode = NgLinearBlendingMode;
    
    _angle = 0.f;
    _gradientType = NgGradientTypeAxial;

    return self;
}

- (void)commonInit {
	_colorspace = [[NgColorSpace genericRGBSpace] retain];
    _elementList = NULL;
}
...
- (id)copyWithZone:(NSZone *)zone {
    NgGradient *copy = [[[self class] allocWithZone:zone] init];
	
    NgGradientElement *currentElement = _elementList;
...
/** @brief Creates and returns a new NgGradient instance initialized with an axial linear gradient between two given colors.
 *  @param begin The beginning color.
 *  @param end The ending color.
 *  @return A new NgGradient instance initialized with an axial linear gradient between the two given colors.
 **/
...
+ (NgGradient *)gradientWithBeginningColor:(NgColor *)begin endingColor:(NgColor *)end angle:(CGFloat)angle {
	NgGradient *ret = [self gradientWithBeginningColor:begin endingColor:end beginningPosition:0.0 endingPosition:1.0];
    if (ret) {
        ret.angle = angle;
    }
    return ret;
...
/** @brief Creates and returns a new NgGradient instance initialized with an axial linear gradient between two given colors, at two given normalized positions.
 *  @param begin The beginning color.
 *  @param end The ending color.
 ...
 *  @return A new NgGradient instance initialized with an axial linear gradient between the two given colors, at two given normalized positions.
 **/
+ (NgGradient *)gradientWithBeginningColor:(NgColor *)begin endingColor:(NgColor *)end beginningPosition:(CGFloat)beginningPosition endingPosition:(CGFloat)endingPosition {
    NgGradient *newInstance = [[self alloc] init];
	
    NgGradientElement color1;
    NgGradientElement color2;
	
	color1.color = NgRGBAColorFromCGColor(begin.cgColor);
	color2.color = NgRGBAColorFromCGColor(end.cgColor);
...
/** @brief Creates and returns a new NgGradient instance initialized with the Aqua selected gradient.
 *  @return A new NgGradient instance initialized with the Aqua selected gradient.
 **/
...
/** @brief Creates and returns a new NgGradient instance initialized with the Aqua normal gradient.
 *  @return A new NgGradient instance initialized with the Aqua normal gradient.
 **/
...
/** @brief Creates and returns a new NgGradient instance initialized with the Aqua pressed gradient.
 *  @return A new NgGradient instance initialized with the Aqua pressed gradient.
 **/
...
/** @brief Creates and returns a new NgGradient instance initialized with the unified selected gradient.
 *  @return A new NgGradient instance initialized with the unified selected gradient.
 **/
...
/** @brief Creates and returns a new NgGradient instance initialized with the unified normal gradient.
 *  @return A new NgGradient instance initialized with the unified normal gradient.
 **/
...
/** @brief Creates and returns a new NgGradient instance initialized with the unified pressed gradient.
 *  @return A new NgGradient instance initialized with the unified pressed gradient.
 **/
...
/** @brief Creates and returns a new NgGradient instance initialized with the unified dark gradient.
 *  @return A new NgGradient instance initialized with the unified dark gradient.
 **/
...
/** @brief Creates and returns a new NgGradient instance initialized with the source list selected gradient.
 *  @return A new NgGradient instance initialized with the source list selected gradient.
 **/
...
/** @brief Creates and returns a new NgGradient instance initialized with the source list unselected gradient.
 *  @return A new NgGradient instance initialized with the source list unselected gradient.
 **/
...
/** @brief Creates and returns a new NgGradient instance initialized with a rainbow gradient.
 *  @return A new NgGradient instance initialized with a rainbow gradient.
 **/
+(NgGradient *)rainbowGradient {
    ...
    newInstance.blendingMode = NgChromaticBlendingMode;
	
    return [newInstance autorelease];
}

/** @brief Creates and returns a new NgGradient instance initialized with a hydrogen spectrum gradient.
 *  @return A new NgGradient instance initialized with a hydrogen spectrum gradient.
 **/
...
- (NgGradient *)gradientWithAlphaComponent:(CGFloat)alpha {
    NgGradient *newGradient = [[[self class] alloc] init];
	
    NgGradientElement *curElement = _elementList;
    NgGradientElement tempElement;
...
- (NgGradient *)gradientWithBlendingMode:(NgGradientBlendingMode)mode {
    NgGradient *newGradient = [self copy];  
 ...
 **/
- (NgGradient *)addColorStop:(NgColor *)color atPosition:(CGFloat)position {
    NgGradient *newGradient = [self copy];
    NgGradientElement newGradientElement;
	
    //put the components of color into the newGradientElement - must make sure it is a RGB color (not Gray or CMYK)
	newGradientElement.color = NgRGBAColorFromCGColor(color.cgColor);
...
- (NgGradient *)removeColorStopAtPosition:(CGFloat)position {
    NgGradient *newGradient = [self copy];
    NgGradientElement removedElement = [newGradient removeElementAtPosition:position];
...
- (NgGradient *)removeColorStopAtIndex:(NSUInteger)index {
    NgGradient *newGradient = [self copy];
    NgGradientElement removedElement = [newGradient removeElementAtIndex:index];
...
- (CGColorRef)newColorStopAtIndex:(NSUInteger)index {
    NgGradientElement *element = [self elementAtIndex:index];
...
- (void)fillRect:(CGRect)rect inContext:(CGContextRef)context {
	...
	switch ( self.gradientType ) {
		case NgGradientTypeAxial:
			myCGShading = [self newAxialGradientInRect:rect];
			break;
		case NgGradientTypeRadial:
...
-(void)setBlendingMode:(NgGradientBlendingMode)mode {
    _blendingMode = mode;
...
- (void)addElement:(NgGradientElement *)newElement {
    if ( _elementList == NULL || newElement->position < _elementList->position ) {
        NgGradientElement *tmpNext = _elementList;
        _elementList = (NgGradientElement*)malloc(sizeof(NgGradientElement));
        if ( _elementList ) {
        ...
        NgGradientElement *tmpNext = curElement->nextElement;
        curElement->nextElement = (NgGradientElement*)malloc(sizeof(NgGradientElement));
...
- (NgGradientElement)removeElementAtIndex:(NSUInteger)index {
    NgGradientElement removedElement;
    ...
    if ( _elementList != NULL ) {
        ...
        while ( currentElement->nextElement != NULL ) {
            if ( count == index ) {
                NgGradientElement *tmpNext  = currentElement->nextElement;
...
-(void)removeAllElements {
    while (_elementList != NULL) {
        NgGradientElement *elementToRemove = _elementList;
        _elementList = _elementList->nextElement;
...
void linearEvaluation (void *info, const CGFloat *in, CGFloat *out) 
{
    ...
    if ( *(NgGradientElement **)info == NULL ) {
        out[0] = out[1] = out[2] = out[3] = 1.0;
        return;
    }
	
    //This grabs the first two colors in the sequence
    NgGradientElement *color1 = *(NgGradientElement **)info;
    NgGradientElement *color2 = color1->nextElement;
...
//	transform the RGB values stored in the NgGradientElements to HSB, in the future I may
                   ...
                   real angle)
{
    ...
    color1.color = NgRGBAColorFromCGColor(begin);
    color2.color = NgRGBAColorFromCGColor(end);
