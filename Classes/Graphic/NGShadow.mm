...
- (id)init {
    ...
    _alpha = .33f;
    self.shadowColor = [NgColor colorWithRGB:0x000000];
...
- (void)setShadowColor:(NgColor *)shadowColor {
...
- (id)initWithCoder:(NSCoder *)coder {
    ...
    _shadowColor = [[NgColor alloc] initWithCoder:coder];
...
- (id)copyWithZone:(NSZone *)zone {
    ...
    shadowCopy.shadowColor = [NgColor colorWithCGColor:_shadowColor.cgColor];
    return shadowCopy;
}
...
- (void)drawInContext:(CGContextRef)ctx {
    ...
    NgColor* tgt_color = [self alphaColor];
    
    NNT_AUTORELEASEPOOL_BEGIN;
    for (uint i = 0; i <= segments;) {
        NgColor* color = [tgt_color colorByMulti:1 g:1 b:1 a:(1 - i * seg_op)];
