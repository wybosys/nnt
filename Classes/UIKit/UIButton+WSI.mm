...
+ (UIButton*)buttonCloseRedRound {
    ...
    WCGImage* pngimg = WCGImageLoadPngData(png_close_red, sizeof(png_close_red));
...
+ (UIButton*)buttonCloseGrayRound {
    UIButton* button = [UIButton buttonWithType:UIButtonTypeCustom];
    WCGImage* pngimg = WCGImageLoadPngData(png_close_gray, sizeof(png_close_gray));
    UIImage *img = [UIImage imageWithCGImage:pngimg.image];
    [button setImage:img forState:UIControlStateNormal];
    button.frame = CGRectMake(0, 0, 30, 30);
    return button;
...
- (void)setTouchedFill:(WCGFill *)touchedFill {
...
- (id)initWithFrame:(CGRect)frame {
    ...
    WCGGradient *backgroundTop = [WCGGradient gradientWithBeginningColor:[WCGColor colorWithRGBA:0x00000000]
                                                             endingColor:[WCGColor colorWithRGBA:0x00000040] 
                                                                   angle:90];            
    WCGGradient *backgroundBottom = [WCGGradient gradientWithBeginningColor:[WCGColor colorWithRGBA:0x00000060]
...
- (void)drawInnerShadow:(CGContextRef)ctx rect:(CGRect)rc {
    ...
    WCGColor *color = [WCGColor colorWithCGColor:innerShadowColor.cgColor];
...
- (void)drawButton:(CGContextRef)ctx rect:(CGRect)rect {
    ...
    if ([_backgroundGradient isKindOfClass:[NSArray class]]) {
        ...
        for (uint i = 0; i < count; ++i) {
            WCGGradient *gradient = (WCGGradient*)[(NSArray*)_backgroundGradient objectAtIndex:i];
            [gradient fillRect:rc inContext:ctx];
            rc.origin.y += comp_height;
        }
    } else if ([_backgroundGradient isKindOfClass:[WCGGradient class]]) {
        [(WCGGradient*)_backgroundGradient fillRect:rect inContext:ctx];
...
- (id)initWithFrame:(CGRect)frame {
    ...
    WCGShapeStyle* style = [[WCGShapeStyle alloc] init];
