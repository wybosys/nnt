...
@implementation UISettingsReader
...
- (void)dealloc {
    safe_release(_path);
    safe_release(_localizationTable);
    safe_release(_bundlePath);
    safe_release(_settingsBundle);
...
- (void)_reinterpretBundle:(NSDictionary*)settingsBundle {
    ...
    for (NSDictionary *specifier in preferenceSpecifiers) {
        if ([(NSString*)[specifier objectForKey:kIASKType] isEqualToString:kIASKPSGroupSpecifier]) {
            ...
            UISettingsSpecifier *newSpecifier = [[UISettingsSpecifier alloc] initWithSpecifier:specifier];
...
- (UISettingsSpecifier*)specifierForIndexPath:(NSIndexPath*)indexPath {
    ...
    UISettingsSpecifier *specifier = [[[self dataSource] objectAtIndex:indexPath.section] objectAtIndex:(indexPath.row+headingCorrection)];
...
- (UISettingsSpecifier*)specifierForKey:(NSString*)key {
    for (NSArray *specifiers in _dataSource) {
        for (id sp in specifiers) {
            if ([sp isKindOfClass:[UISettingsSpecifier class]]) {
