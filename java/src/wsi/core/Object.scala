...
class Object {
    ...
    private type event_type = wsi.core.Event        
    ...
    def connect(sig:signal_t, sel:method_t, hdl:wsi.core.Object) :slot_t = this.event().connect(sig, sel, hdl)
    
    // traces.
    def trace[T](obj:T) = wsi.WSI.trace(obj)
